...
parser.add_argument('--ngpu', type=str, default='1', 
...
def train(model, criterion, trainData, optim, epoch):
    ...
    for i in range(len(trainData)):
        ...
        for j in range(0, target_size-1, trunc_size):
            ...
            # Main training loop
            model.zero_grad()
            outputs, attn, dec_state = model(trunc_batch.src, trunc_batch.tgt, trunc_batch.lengths, dec_state)
...
def main():
    ...
    trainData = onmt.Dataset(dataset['train']['src'],
                             dataset['train']['tgt'], opt.batch_size, opt.ngpu,
    ...
    validData = onmt.Dataset(dataset['valid']['src'],
                             dataset['valid']['tgt'], opt.batch_size, opt.ngpu,
                             volatile=True,
                             data_type=dataset.get("type", "text"),
    ...
    model = onmt.Models.NMTModel(encoder, decoder, opt.ngpu > 1)
    ...
    if opt.ngpu > 1:
        opt.gpu_ids = [1, 2]
        print('Multi gpu training ', opt.ngpu)
        model = nn.DataParallel(model, device_ids=[1, 2], dim=1)
        generator = nn.DataParallel(generator, device_ids=[1, 2], dim=0)
    ...
    best_ppl = 0 # or we just use best_acc
    # debug_here()
    for epoch in range(opt.start_epoch, opt.epochs + 1):  
        print('Epoch {0} / {1}'.format(epoch, opt.epochs + 1))   
        # (1) train model for one epoch 
        train_stats = train(model, criterion, trainData, optim, epoch)
        ...
        model_state_dict = (model.module.state_dict() if opt.ngpu > 1 else model.state_dict())
        ...
        if epoch >= opt.start_checkpoint_at and best_ppl < valid_stats.ppl():
            ...
            model_path = os.path.join(opt.checkpoint_folder, 'model_ppl_best.pt')
